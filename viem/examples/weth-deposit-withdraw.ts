...
import { ChainInstance, WETH, ERC20 } from '../index';
...
async function main() {
    ...
    // Setup
    const chain = base;
    const kit = ChainInstance.for(chain);
    ...
    try {
        ...
        const initialWethBalance = await ERC20.balanceOf(publicClient, chain.contracts?.wrappedEther?.address!, account.address);
        
        console.log(`   ETH: ${initialEthBalance / parseEther('1')} ETH`);
        console.log(`   WETH: ${kit.formatErc20Amount(initialWethBalance, chain.contracts?.wrappedEther?.address!)} WETH`);

        // 2. Deposit 0.01 ETH to WETH
        const depositAmount = '0.01';
        console.log(`\n🔄 Depositing ${depositAmount} ETH to WETH...`);
        
        const depositReceipt = await WETH.deposit(
            publicClient,
            walletClient,
            kit,
            depositAmount
        ...
        const afterDepositWethBalance = await ERC20.balanceOf(publicClient, chain.contracts?.wrappedEther?.address!, account.address);
        
        console.log(`   ETH: ${afterDepositEthBalance / parseEther('1')} ETH`);
        console.log(`   WETH: ${kit.formatErc20Amount(afterDepositWethBalance, chain.contracts?.wrappedEther?.address!)} WETH`);
        ...
        const withdrawReceipt = await WETH.withdraw(
            publicClient,
            walletClient,
            kit,
            withdrawAmount
        ...
        const finalWethBalance = await ERC20.balanceOf(publicClient, chain.contracts?.wrappedEther?.address!, account.address);
        
        console.log(`   ETH: ${finalEthBalance / parseEther('1')} ETH`);
        console.log(`   WETH: ${kit.formatErc20Amount(finalWethBalance, chain.contracts?.wrappedEther?.address!)} WETH`);
