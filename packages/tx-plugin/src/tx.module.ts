	...
	async sendTx(
		...
		if (txOptions?.enableGasPrice ?? this.option?.enableGasPrice) {
			...
			if (gasPriceOverrides.maxPriorityFeePerGas) {
				ptx.maxPriorityFeePerGas = ethers.BigNumber.from(
	...
	async batchSendTxs(
		...
		try {
			...
			const txs = await Promise.all(
				_rawTxs.map(async (rawTx, i) => await signers[i].populateTransaction(rawTx)),
			);
			...
			await Promise.all(
				receipts.map(
					async (receipt, i) => await this.handleReceipt(receipt, (await rawTxs[i]).gasLimit),
				),
			);
			return receipts;
		} catch (errs) {
		...
		try {
			...
			if (parsed.length === 1) {
				...
				this.logger.info(
					...
					chalk.green(parsedContractAddr + '::' + parsed[0].substring(0, idx)) +
