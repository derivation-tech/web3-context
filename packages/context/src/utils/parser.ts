...
export class ContractParser {
	...
	constructor(
		interface_: ethers.utils.Interface,
		addressParser?: (address: string) => Promise<string>,
	) {
			...
			if (paramType.components) {
				for (const item of value) {
					...
					for (const subParamType of paramType.components) {
						str +=
							(await this.parseParam(description, subParamType, item[subParamType.name])) + ', ';
					}
					str = str.slice(0, -2);
					str += '),';
				}
	...
	protected async parseFunction(description: ethers.utils.TransactionDescription): Promise<string> {
		...
		const value = description.value.gt(0)
			? `{value: ${ethers.utils.formatEther(description.value)}}`
			: '';
	...
	public async parseTransaction(tx: {
		data: string;
		...
		if (!description)
			throw new ContractParserError(`Invalid transaction data ${tx.data.toString()}`);
...
export class UniswapV2PairParser extends ContractParser {
	...
	constructor(
		...
		this.tokenAIsToken0 =
			tokenATokenInfo.address.toLowerCase() < tokenBTokenInfo.address.toLowerCase();
	}
