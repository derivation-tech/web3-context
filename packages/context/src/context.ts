...
export class Context implements BaseContext {
	...
	public setProvider(provider: Provider, isOpSdkCompatible?: boolean) {
		...
		this.wrappedNative = WrappedNative__factory.connect(
		...
		this.registerContractParser(
			...
			new ContractParser(this.multiCall3.interface),
		);
		this.registerContractParser(
			this.info.tokenAssistant,
			new ContractParser(this.tokenAssistant.interface),
		);
	...
	async balanceOfTokenBatch(
		...
		if (tokens.length > batch)
			throw new ContextCoreError(`${tokens.length} exceeds batch size ${batch}`);
		...
		const bs = await Promise.all(
			chunks.map(chunk =>
				this.retry(() => this.tokenAssistant.balanceOfTokenBatch(tokens, chunk, overrides)),
			),
		);
