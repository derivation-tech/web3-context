...
export async function getRevertReason(
	...
	try {
		...
		await retry(async () => {
			try {
				codeOrMsg = await extractRevertData(
					provider,
					txHashOrResponse as ethers.providers.TransactionResponse,
				);
			} catch (e: any) {
				if (
		...
		const reason =
			codeOrMsg && codeOrMsg.startsWith('0x') ? decodeRevertData(codeOrMsg) : codeOrMsg;
		return reason;
	} catch (err) {
		logger.error(err);
...
export async function extractRevertData(
	provider: Provider,
